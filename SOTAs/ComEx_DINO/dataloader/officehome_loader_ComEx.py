...
def OfficeHomeLoader(args, lab_flag, domain, split='train', transform_mode = None, shuffle = None, label_range=range(5)): 
    if args.stage == 'pretrain':
        if transform_mode == 'train':
            transform = get_transforms(args, "unsupervised", 'OfficeHome', args.num_views)
        elif transform_mode == 'test':
            transform = get_transforms(args, "eval", 'OfficeHome', args.num_views)
...
def get_OfficeHome_loader(args):
    ...
    data_loader = dict()
    data_loader['train_lab_loader'] = OfficeHomeLoader(args, 'lab', args.source_domain, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_base_classes))
    data_loader['train_unlab_loader'] = OfficeHomeLoader(args, 'unlab', args.target_domain, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_base_classes, args.num_classes))
    data_loader['test_lab_loader'] = OfficeHomeLoader(args, 'lab', args.source_domain, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes))
    data_loader['test_unlab_loader'] = OfficeHomeLoader(args, 'unlab', args.target_domain, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes, args.num_classes))
    # HACK in discover stage use 'Evaluate on unlabeled classes (train split)' add transform_mode='test'
    data_loader['train_unlab_loader2'] = OfficeHomeLoader(args, 'unlab', args.target_domain, split='train', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes, args.num_classes))
    return data_loader