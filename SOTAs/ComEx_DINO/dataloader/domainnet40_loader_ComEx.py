# load domainnet40 data with cross domain or one domain
...
class DOMAINNET(data.Dataset):
    ...
    def __init__(self, args, lab_flag, domain_name, split='train + test', transform=None, label_range = range(5)):
        self.args = args
        self.data_root = args.dataset_root # data_root
        ...
        if lab_flag == 'lab':
            txt_path = os.path.join(self.data_root, 'image_list', domain_name + '_' + split + '_mini.txt')
        elif lab_flag == 'unlab':
            txt_path = os.path.join(self.data_root, 'image_list', domain_name + '_' + split + '_mini.txt')
        else:
...
def DOMAINNETLoader(args, lab_flag, domain, split='train', transform_mode = None, shuffle = None, label_range=range(5)):
    if args.stage == 'pretrain':
        if transform_mode == 'train':
            transform = get_transforms(args, "unsupervised", 'DomainNet', args.num_views)
        elif transform_mode == 'test':
            transform = get_transforms(args, "eval", 'DomainNet', args.num_views)
        ...
        transform = get_transforms(args, "eval", 'DomainNet')
    else:
        raise ValueError('stage should be pretrain or discover')
    dataset = DOMAINNET(args, lab_flag, domain, split=split, transform = transform, label_range=label_range)
...
def get_DomainNet_loader(args):
    # seperately load two domain data (lab and unlab)
    data_loader = dict()
    data_loader['train_lab_loader'] = DOMAINNETLoader(args, 'lab', args.source_domain, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_base_classes))
    data_loader['train_unlab_loader'] = DOMAINNETLoader(args, 'unlab', args.target_domain, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_base_classes, args.num_classes))
    data_loader['test_lab_loader'] = DOMAINNETLoader(args, 'lab', args.source_domain, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes))
    data_loader['test_unlab_loader'] = DOMAINNETLoader(args, 'unlab', args.target_domain, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes, args.num_classes))
    # HACK in discover stage use 'Evaluate on unlabeled classes (train split)' add transform_mode='test'
    data_loader['train_unlab_loader2'] = DOMAINNETLoader(args, 'unlab', args.target_domain, split='train', transform_mode='test', shuffle=False, label_range = range(args.num_base_classes, args.num_classes))
    return data_loader