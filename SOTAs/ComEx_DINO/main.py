...
def main(args, logger):
    ...
    if args.dataset_series == 'DomainNet':
        from dataloader.domainnet40_loader_ComEx import get_DomainNet_loader
        data_loader = get_DomainNet_loader(args)
    elif args.dataset_series in ('CIFAR10', 'CIFAR100'):
        from dataloader.cifar_loader import get_CIFAR_loader
        data_loader = get_CIFAR_loader(args)
    elif args.dataset_series == 'OfficeHome':
        from dataloader.officehome_loader_ComEx import get_OfficeHome_loader
...
parser = argparse.ArgumentParser(description='ComEx')
...
parser.add_argument('--total_name', type=str, default='NCD_SOTA_ComEx') # total name for save the trained model
parser.add_argument('--current_name', type=str, default='ComEx_UNO')
parser.add_argument('--stage', type=str, default='discover', choices=['pretrain','discover','test'])
...
# optimzer parameters
parser.add_argument('--base_lr', default=0.2, type=float, help='initial learning rate')
parser.add_argument('--min_lr', default=0.001, type=float, help="min learning rate")
parser.add_argument('--momentum_opt', default=0.9, type=float, help="momentum for optimizer")
parser.add_argument('--weight_decay_opt', default=1e-4, type=float, help='weight decay (default: 1e-4)')
...
parser.add_argument('--target_domain', type=str, default='sketch',choices=['sketch','clipart','painting','real','Real_World','Art', 'Clipart', 'Product']) # for DomainNet, OfficeHome
...
parser.add_argument('--num_base_classes', default=5, type=int)
parser.add_argument('--num_novel_classes', default=5, type=int)
...
parser.add_argument("--batch_head", default=False, action="store_true", help="whether to use batch-wise experts") # HACK
parser.add_argument("--batch_head_multi_novel", default=False, action="store_true", help="whether to use multi heads in novel-batch expert")# HACK

# specail parameters for ComEx discover stage
...
parser.add_argument("--sharp", default=0.5, type=float, help="for sharpening the target distribution, lower is sharper")
parser.add_argument("--magic", default=False, action="store_true", help="use dim=1 in ce loss")

# SimGCD parameters
parser.add_argument('--grad_from_block', type=int, default=11) #HACK 
