...
class Solver(object):
    ...
    def build_pretrain_model(self):
        self.pretrain_model = MultiHeadModel(
            ...
            num_base=self.args.num_base_classes,
            num_novel=self.args.num_novel_classes,
            num_heads=None,
        )
        self.pretrain_model.cuda(self.args.gpu)
        self.print_network(self.pretrain_model, 'ComEx_{}'.format(self.args.arch))
    ...
    def build_discover_model(self):
        self.discover_model = MultiHeadModel(
            ...
            batch_head=self.args.batch_head,
            batch_head_multi_novel=self.args.batch_head_multi_novel
        )
        ...
        self.print_network(self.discover_model, 'ComEx_{}'.format(self.args.arch))
    ...
    def restore_discover_for_test(self, model_saved_path):
        self.build_discover_model()
        # load trained discover model
        checkpoint = torch.load(model_saved_path, map_location=lambda storage, loc: storage.cuda(self.args.gpu))
        self.discover_model.load_state_dict(checkpoint['model_state_dict'], strict=False)

        file_stat = os.stat(model_saved_path)
        mtime = file_stat.st_mtime
        saved_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(mtime))
        self.logger.info("=> loaded trained discover model '{}', this model saved time is {} ".format(model_saved_path, saved_time))
    ...
    def test(self):
        ...
        if self.args.test_matrics:
            ...
            discover.test(self.discover_model, self.args, self.logger, 'base/test', self.data_loader['test_lab_loader'])

            self.logger.info('Evaluate on unlabeled classes (train split)')
            discover.test(self.discover_model, self.args, self.logger, 'novel/train', self.data_loader['train_unlab_loader2'])

            self.logger.info('Evaluate on unlabeled classes (test split)')
            discover.test(self.discover_model, self.args, self.logger, 'novel/test', self.data_loader['test_unlab_loader'])
