...
def train(model, data_loader, args, loggers):
    train_labeled_loader = data_loader['train_lab_loader']
    # init wandb
    if args.use_wandb:
        #======================================================================
    ...
    for epoch in range(args.epochs):
        ...
        for _, (labeled_imgs, labeled_label, _) in enumerate(tqdm(train_labeled_loader)):
            
            imgs = [labeled_img.cuda(args.gpu, non_blocking=True) for labeled_img in labeled_imgs] # num_view transform produce num_view imgs to form a list
            ...
            # supervised loss
            loss_supervised = torch.stack([F.cross_entropy(o / args.softmax_temperature, label) for o in outputs["logits_base"]]).mean()
