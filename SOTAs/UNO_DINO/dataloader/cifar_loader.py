...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, 
                ...
                download=False, 
                label_range = range(5)):
        self.root = args.dataset_root # dataset path
...
def CIFARLoader(args, split='train', transform_mode = None, shuffle = None, label_range=range(5)):
    if args.stage == 'pretrain':
        if transform_mode == 'train':
            transform = get_transforms(args, "unsupervised", args.dataset_series)
        elif transform_mode == 'test':
            transform = get_transforms(args, "eval", args.dataset_series)
...
def get_CIFAR_loader(args):
    # process both cifar10
    # seperately load two part data (labeled and unlabeled)
    data_loader = dict()
    # labeled data
    if args.dataset_subclass in ['CIFAR10CMix','new_select']: # cifar10 labelled data
        data_loader['train_lab_loader'] = CIFARLoader(args, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_labeled_classes))
        data_loader['test_lab_loader'] = CIFARLoader(args, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes))
    elif args.dataset_subclass in ['CIFAR10CAll']:
        data_loader['train_lab_loader'] = CIFAR10CLoader(args, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_labeled_classes))
        data_loader['test_lab_loader'] = CIFAR10CLoader(args, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes))
    else:
        raise NotImplementedError
    # unlabeled data
    if args.dataset_subclass in ['CIFAR10CMix','CIFAR10CAll']:
        data_loader['train_unlab_loader'] = CIFAR10CLoader(args, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_labeled_classes, args.num_classes))
        data_loader['test_unlab_loader'] = CIFAR10CLoader(args, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
        # HACK in discover stage use 'Evaluate on unlabeled classes (train split)' add transform_mode='test'
        data_loader['train_unlab_loader2'] = CIFAR10CLoader(args, split='train', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
    elif args.dataset_subclass in ['new_select']: # cifar10 unlabelled data
        data_loader['train_unlab_loader'] = CIFARLoader(args, split='train', transform_mode='train', shuffle=True, label_range = range(args.num_labeled_classes, args.num_classes))
        data_loader['test_unlab_loader'] = CIFARLoader(args, split='test', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
        # HACK in discover stage use 'Evaluate on unlabeled classes (train split)' add transform_mode='test'
        data_loader['train_unlab_loader2'] = CIFARLoader(args, split='train', transform_mode='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
