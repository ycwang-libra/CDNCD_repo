...
import numpy as np
import pickle
from util.util import check_integrity, download_url, Solarize, Equalize
...
def train_transform_lab(args):
    ...
    transform = loader.TwoCropsTransform(
            (T.Compose(augmentation)))
    return transform

def train_transform_unlab(args):
    augmentation = [
        T.RandomChoice([T.RandomCrop(args.resizecrop_size, padding=4),
                        T.RandomResizedCrop(args.resizecrop_size, (0.5, 1.0))]),
        T.RandomHorizontalFlip(),
...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, 
        ...
        for file_name, checksum in downloaded_list:
            file_path = os.path.join(self.root, self.base_folder, file_name) # '/datasets/CIFAR/cifar-10-batches-py/data_batch_1'  data_batch_1,2,3,4,5
...
class CIFAR10C(data.Dataset):
    def __init__(self, args, split='train+test', transform=None, label_flag = None, label_range = range(5)):
        self.args = args
        self.transform = transform
        self.cifar10c_data = np.load(self.args.dataset_root + '/CIFAR-10_corruption/' + split + '/Severity' + str(self.args.unlab_corrupt_severity if label_flag == 'unlab' else self.args.lab_corrupt_severity) + '/' + self.args.corrupt_mode + '.npy')
...
def CIFARLoader(args, split='train', shuffle = None, label_range=range(5)):
    if split == 'train':
        if label_range == range(args.num_labeled_classes):
            transform = train_transform_lab(args)
        elif label_range == range(args.num_labeled_classes, args.num_classes):
            transform = train_transform_unlab(args)
        else:
...
def CIFAR10CLoader(args, split='train', shuffle = None, label_flag = None, label_range=range(5)):
    if split == 'train':
        if label_range == range(args.num_labeled_classes):
            transform = train_transform_lab(args)
        elif label_range == range(args.num_labeled_classes, args.num_classes):
    ...
    dataset = CIFAR10C(args, split=split, transform = transform, label_flag = label_flag, label_range=label_range)
...
def get_CIFAR_loader(args):
    ...
    if args.dataset_subclass in ['CIFAR10CMix','new_select']: # cifar10 or cifar100 labelled data
        ...
        data_loader['train_lab_loader'] = CIFAR10CLoader(args, split='train', shuffle=True, label_flag = 'lab', label_range = range(args.num_labeled_classes))
        data_loader['test_lab_loader'] = CIFAR10CLoader(args, split='test', shuffle=False, label_flag = 'lab', label_range = range(args.num_labeled_classes))
    else:
        raise NotImplementedError
    # unlabeled data
    if args.dataset_subclass in ['CIFAR10CMix','CIFAR10CAll']:
        data_loader['train_unlab_loader'] = CIFAR10CLoader(args, split='train', shuffle=True, label_flag = 'unlab', label_range = range(args.num_labeled_classes, args.num_classes))
        data_loader['test_unlab_loader'] = CIFAR10CLoader(args, split='test', shuffle=False, label_flag = 'unlab', label_range = range(args.num_labeled_classes, args.num_classes))
