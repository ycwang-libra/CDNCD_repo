...
def DOMAINNETLoader(args, lab_flag, domain, split='train', shuffle = None, label_range=range(5)):
    if split == 'train':
...
def get_DomainNet_loader(args):
    ...
    data_loader = dict()
    data_loader['train_lab_loader'] = DOMAINNETLoader(args, 'lab', args.source_domain, split='train', shuffle=True, label_range = range(args.num_labeled_classes))
    data_loader['train_unlab_loader'] = DOMAINNETLoader(args, 'unlab', args.target_domain, split='train', shuffle=True, label_range = range(args.num_labeled_classes, args.num_classes))
    data_loader['test_lab_loader'] = DOMAINNETLoader(args, 'lab', args.source_domain, split='test', shuffle=False, label_range = range(args.num_labeled_classes))
    data_loader['test_unlab_loader'] = DOMAINNETLoader(args, 'unlab', args.target_domain, split='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
    return data_loader