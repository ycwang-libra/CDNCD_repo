...
class OfficeHome(data.Dataset):
    ...
    def __init__(self, args, lab_flag, domain_name, split='train + test', transform=None, label_range = range(5)):
        ...
        if lab_flag == 'lab':
            ...
            if args.unlab_corrupt_severity == 0:
                txt_path = os.path.join(self.data_root, 'image_list', domain_name + '_' + split + '.txt')
            else:
...
def get_OfficeHome_loader(args):
    ...
    data_loader = dict()
    data_loader['train_lab_loader'] = OfficeHomeLoader(args, 'lab', args.source_domain, split='train', shuffle=True, label_range = range(args.num_labeled_classes))
    data_loader['train_unlab_loader'] = OfficeHomeLoader(args, 'unlab', args.target_domain, split='train', shuffle=True, label_range = range(args.num_labeled_classes, args.num_classes))
    data_loader['test_lab_loader'] = OfficeHomeLoader(args, 'lab', args.source_domain, split='test', shuffle=False, label_range = range(args.num_labeled_classes))
    data_loader['test_unlab_loader'] = OfficeHomeLoader(args, 'unlab', args.target_domain, split='test', shuffle=False, label_range = range(args.num_labeled_classes, args.num_classes))
    return data_loader